...
from hloc_src.hloc import extract_features, match_features, reconstruction,  pairs_from_retrieval, pairs_from_exhaustive
...
async def extract_match_features(uuid: UUID):
    try:
        ...
        if _data[4]:
            return ORJSONResponse(content={"status": "Map already Uploaded, No need of generation"})
        if _data[3]:
        ...
        if _no_images > 0:
            ...
            retrieval_path = extract_features.main(retrieval_conf,
                                                   _session_dataset_mapping,
                                                   feature_path=_netvlad_features
                                                   )
            pairs_from_retrieval.main(retrieval_path, _sfm_pairs, num_matched=100)
            extract_features.main(feature_conf, _session_dataset_mapping, feature_path=_features)
            match_features.main(matcher_conf, _sfm_pairs, features=_features, matches=_matches)
            return ORJSONResponse(content={"status": "Extraction, Matching ran successfully and h5's saved"})
        else:
            return ORJSONResponse(status_code=404, content={"status": "No Images found in the database"})
...
async def generate_map(uuid: UUID):
    try:
        ...
        if _no_images > 0:
            ...
            _sfm_pairs = _session_outputs / SFM_PAIRS
            _sfm_dir = _session_outputs / SFM_DIR
            reconstruction.main(_sfm_dir, _session_dataset_mapping, _sfm_pairs, _features, _matches)
            _update_q = update(UUIDS).where(UUIDS.c.uuid == str(uuid)).values(map_generated=True)
            await DATABASE.execute(_update_q)
            return ORJSONResponse(content={"status": "SFM generated successfully and"
                                                     "saved"})
        else:
            return ORJSONResponse(status_code=404, content={"status": "No Images found in the database"})
    except Exception as e:
